...
public abstract class Player implements Comparable<Player> {
    ...
    @Override
     public int compareTo(Player p) {
        int currFrozen = this.isFrozen() ? 1 : 0;
        int nextFrozen = p.isFrozen() ? 1 : 0;
        
        // if both are frozen or both are unfrozen, use sus level to sort
        if (currFrozen == nextFrozen) {
            return this.susLevel - p.susLevel;
        }
        // sort frozen players on the left, unfrozen players on the right
        return nextFrozen - currFrozen;
    }
